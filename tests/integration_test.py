...
class StressTests(unittest.TestCase):
    ...
    def test_extreme_data_values(self):
        """Test components with large data values.

        MI and CMS use counting-sort (allocates max+1 cells), so INT32_MAX
        is not feasible.  Use large-but-allocatable values instead.
        """
        max_val = 100_000
        min_val = 0  # counting-sort requires non-negative values
